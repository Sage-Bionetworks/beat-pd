...
files<-list.files("Analyses/Features/cis-solly-splits/", full.names = TRUE)
...
ggsave("Analyses/Plots/CIS-PD_on_off_weighting_Alexs_models_v2.png", height=5.5, width=10)
...
write.csv(on_off_summary, "Analyses/CIS-PD_on_off_weighting_Alexs_models_v2.csv", row.names=F, quote=F)
...
ggsave("Analyses/Plots/CIS-PD_dyskinesia_weighting_Alexs_models_v2.png", height=5.5, width=10)
...
write.csv(dyskinesia_summary, "Analyses/CIS-PD_dyskinesia_weighting_Alexs_models_v2.csv", row.names=F, quote=F)
...
ggsave("Analyses/Plots/CIS-PD_tremor_weighting_Alexs_models_v2.png", height=5.5, width=10)
...
write.csv(tremor_summary, "Analyses/CIS-PD_tremor_weighting_Alexs_models_v2.csv", row.names=F, quote=F)

save.image("../Code/CIS-PD Modeling/Modeling_Solly/CIS-PD_Scoring_Weighting_v2.Rdata")
...
ggplot(on_off_res)+geom_boxplot(aes(x=Model, y=lift))+facet_grid(Stat~Weight) + geom_point(data=filter(on_off_res, split_id==mod), aes(x=Model, y=lift), position=position_dodge(width=0.75), col=2)+ theme(axis.text.x = element_text(angle = 90))+ geom_hline(yintercept=0, linetype="dashed", color = "red")
ggsave("Analyses/Plots/CIS-PD_on_off_weighting_Alexs_models_7_v2.png", height=5.5, width=10)
ggplot(dyskinesia_res)+geom_boxplot(aes(x=Model, y=lift))+facet_grid(Stat~Weight) + geom_point(data=filter(dyskinesia_res, split_id==mod), aes(x=Model, y=lift), position=position_dodge(width=0.75), col=2)+ theme(axis.text.x = element_text(angle = 90))+ geom_hline(yintercept=0, linetype="dashed", color = "red")
ggsave("Analyses/Plots/CIS-PD_dyskinesia_weighting_Alexs_models_7_v2.png", height=5.5, width=10)
ggplot(tremor_res)+geom_boxplot(aes(x=Model, y=lift))+facet_grid(Stat~Weight) + geom_point(data=filter(tremor_res, split_id==mod), aes(x=Model, y=lift), position=position_dodge(width=0.75), col=2)+ theme(axis.text.x = element_text(angle = 90))+ geom_hline(yintercept=0, linetype="dashed", color = "red")
ggsave("Analyses/Plots/CIS-PD_tremor_weighting_Alexs_models_7_v2.png", height=5.5, width=10)
...
ggplot(subset(res_ind, model=="regress-xg"), aes(x=subject_id, y=Lift)) + geom_boxplot() + 
  facet_grid(Statistic~Phenotype) + theme(axis.text.x = element_text(angle = 90)) +
  xlab("Subject ID") + ylab("Lift") + 
  stat_summary(aes(x=subject_id, y=n_total), fun.data = stat_box_data, geom = "text", size = 3, hjust = 0.5, vjust=0.9, angle=90, position = position_dodge(width = 0.8),  fun.args = list(lower_limit=min(res_ind$Lift[res_ind$model=="regress-xg"])*1.15)) +
  geom_hline(yintercept=0, linetype="dashed", color = "red")
ggsave("Analyses/Plots/CIS-PD_AlexRegressXG_mse_mae.png", width = 8, height = 6.5)
